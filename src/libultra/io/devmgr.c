    ...
    while (true) {
        ...
        label:
            ...
            switch (ioMesg->hdr.type) {
                ...
                case 15:
                    osRecvMesg(devMgr->acsQueue, &sp6C, OS_MESG_BLOCK);
                    ret = devMgr->edma(ioMesg->piHandle, OS_READ, ioMesg->devAddr, ioMesg->dramAddr, ioMesg->size);
                    break;
                case 16:
                    osRecvMesg(devMgr->acsQueue, &sp6C, OS_MESG_BLOCK);
                    ret = devMgr->edma(ioMesg->piHandle, OS_WRITE, ioMesg->devAddr, ioMesg->dramAddr, ioMesg->size);
                    break;
                case 10:
